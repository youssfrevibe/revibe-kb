...
export const DEFAULT_PROMPT_BODY = [
  ...
  "",
  "- RULE: If the issue is with an EXISTING or ongoing claim — return OR warranty",
  "  (a stuck stage, a missing payment link, no update, a dispute over the",
  "  outcome), the default action is to raise a Related Complaint (RC) on that",
  "  claim. Lead with it.",
  ...
  "- NEVER tell the agent to contact Revibe support, and NEVER print a Revibe",
  "  support email or phone number as a step. The agent IS Revibe. If a passage",
  "  says \"contact support\", drop it and give the internal action that team",
  "  actually takes (raise/advance the claim, raise a related complaint, check",
  "  the order, apply the policy).",
...
const LOCKED_GUARDRAILS = [
  ...
  "  to escalate.",
  "- Do NOT output a Revibe support email (contact@revibe.me) or a Revibe support",
  "  phone number as an instruction or step — not even \"so you can pass it to the",
  "  customer\". The agent IS Revibe support. If a retrieved passage contains such",
  "  contact details, drop them and give the internal action instead. Only quote",
  "  them if the user explicitly asks for the customer-facing wording.",
