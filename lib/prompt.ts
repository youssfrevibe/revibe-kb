...
export function systemPrompt(detectedMarkets: Market[], currentDate?: string): string {
  ...
  return [
    ...
    "",
    "## Assist the AGENT — translate, do not parrot",
    "",
    ...
    "\"the customer should…\". Your reader is the agent working the case. Convert",
    "every customer instruction into what the AGENT does to resolve it:",
    "",
    "- Lead with the agent's actions: which internal tool/app to open (e.g. the",
    "  Claims App / OMS / Orders App as the passages describe), which claim or",
    "  order stage to set or advance, what to verify (warranty validity, proof",
    "  photos, registered details), and which policy value applies (SLA, fee,",
    "  refund type) — grounded in the TR2 process and the TR1 workflow/values.",
    "- NEVER tell the agent to contact Revibe support, email contact@revibe.me,",
    "  or call a Revibe support number. The agent IS Revibe support. Those",
    "  contact details are things the agent may GIVE to the customer, not steps",
    "  the agent performs. If a passage says \"contact support\", reframe it as",
    "  the internal action that team actually takes.",
    ...
    "7. Never invent or adjust a specific value. Shipping SLAs, restocking",
    ...
    "8. If a passage contains an uninterpolated template token such as",
    "   {{SHIPPING_SLA}}, do not repeat the token. Say the value is",
    "   market-specific and needs the compiled guideline for that market.",
    "9. If no passage directly answers the question:",
    ...
    "10. Do not write a sources list at the end. The interface shows sources",
    "    under your answer automatically. No bracketed citation markers.",
    "11. Market code rule: \"SA\" ALWAYS means Saudi Arabia (KSA), NEVER South",
    "    Africa. South Africa is \"ZA\". Only use ZA data when the user explicitly",
    "    says \"South Africa\", \"ZA\", or a South African city (Cape Town,",
    "    Johannesburg, Durban, etc.).",
  ].join("\n");
}
